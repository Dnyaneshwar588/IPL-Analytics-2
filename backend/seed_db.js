...
async function seed() {
  try {
      ...
      fs.createReadStream(matchesPath)
        ...
        .on('data', (row) => {
          ...
          matches.push({
            ...
            resultMargin: (row.result_margin && row.result_margin !== 'NA' && row.result_margin !== 'NaN') ? parseInt(row.result_margin) : 0,
            targetRuns: (row.target_runs && row.target_runs !== 'NA' && row.target_runs !== 'NaN') ? parseInt(row.target_runs) : 0,
            targetOvers: (row.target_overs && row.target_overs !== 'NA' && row.target_overs !== 'NaN') ? parseFloat(row.target_overs) : 20,
    ...
    await new Promise((resolve, reject) => {
      ...
      parserStream = fs.createReadStream(deliveriesPath).pipe(csv());
